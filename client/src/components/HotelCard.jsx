...
const HotelCard = ({room, index}) => {
  return (
    <Link to={'/rooms/' +room._id} onClick={()=> scrollTo(0,0)} key={room._id}>
        <img src={room.images[0]} alt='' className='relative max-w-70 w-full [0px_4px_4px_rgba(0,0,0,0.05)]'/>  
        ...
        <div className='p-4 pt-5'>
            <div className='flex items-center justify-between'>
                <p className='font-playfair text-xl font-medium text-gray-800'>
                    {room.hotel.name}</p>
                <div className='flex iteks-center gap-1'>
            ...
            <div className='flex item-center justify-between mt-4'>
                ...
                <button className='px-4 py-2 text-sm font-medium border norder-gray-300 rounded hover:bg-gray-50 transition-all cursor-pointer'>Book Now</button>
            </div>
        </div>
